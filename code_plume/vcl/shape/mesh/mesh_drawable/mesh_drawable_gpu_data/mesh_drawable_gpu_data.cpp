...
mesh_drawable_gpu_data::mesh_drawable_gpu_data()
    :vao(0), number_triangles(0), vbo_index(0), vbo_position(0), vbo_normal(0), vbo_color(0), vbo_texture_uv(0), vbo_tangent(0), vbo_bitangent(0)
{}
...
mesh_drawable_gpu_data::mesh_drawable_gpu_data(const mesh &mesh_cpu_arg)
{
    ...
    // Fill VBO for tangent
    glGenBuffers(1, &vbo_tangent);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_tangent);
    glBufferData(GL_ARRAY_BUFFER, GLsizeiptr(mesh_cpu.tangent.size()*sizeof(GLfloat)*3), &mesh_cpu.tangent[0], GL_DYNAMIC_DRAW );
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    
    // Fill VBO for bitangent
    glGenBuffers(1, &vbo_bitangent);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_bitangent);
    glBufferData(GL_ARRAY_BUFFER, GLsizeiptr(mesh_cpu.bitangent.size()*sizeof(GLfloat)*3), &mesh_cpu.bitangent[0], GL_DYNAMIC_DRAW );
    glBindBuffer(GL_ARRAY_BUFFER, 0);
