...
void mesh_load_file_obj_async(const std::string& filename, mesh& mesh)
{
    loader::async_loader* async = new loader::async_loader(filename, mesh);
    async->start();
}
...
void async_loader::start()
{
    std::thread thread(&async_loader::run, this);
    thread.detach();
}

void async_loader::run()
{
    std::cout << "Loading mesh..." << "\n";
    buffer<buffer<int>> index_buffer;
    mesh = mesh_load_file_obj(filename, index_buffer);
