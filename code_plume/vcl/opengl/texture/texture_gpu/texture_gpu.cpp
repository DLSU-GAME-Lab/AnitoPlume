...
GLuint create_texture_cube_map_gpu(std::vector<image_raw> const& im)
{
    GLuint id = 0;
    glGenTextures(1, &id);
...
GLuint add_normal_map(image_raw const& im, GLint wrap_s, GLint wrap_t)
{
    return add_normal_map(im.data, GLsizei(im.width), GLsizei(im.height), wrap_s, wrap_t);
}

GLuint add_normal_map(std::vector<unsigned char> const& data, GLsizei width, GLsizei height, GLint wrap_s, GLint wrap_t)
{
    GLuint id = 0;
    glGenTextures(1, &id);
    glBindTexture(GL_TEXTURE_2D, id);
